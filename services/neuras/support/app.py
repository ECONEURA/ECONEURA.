"""
ECONEURA Support Agent (neura-11)
...
# Inicializar FastAPI
app = FastAPI(
    title="ECONEURA Support Agent",
    version="1.0.0",
    description="Agent specialized in Support Agent responsibilities"
)
...
tracer, meter = setup_observability("neura-support", "1.0.0")
instrument_fastapi_app(app, "neura-support")

# Métricas
request_counter = meter.create_counter(
    "support_requests_total",
    description="Total requests to Support Agent"
)
error_counter = meter.create_counter(
    "support_errors_total",
    description="Total errors in Support Agent"
)
...
async def execute_task(
    ...
    """
    with tracer.start_as_current_span("support_task") as span:
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s)
                RETURNING id
            """, ("neura-11", task.user_id, json.dumps(task.input), task.correlation_id, "processing"))
                ...
                response = await client.post(
                    ...
                    json={
                        "messages": [
                            {
                                "role": "system",
                                "content": "You are an technical support and troubleshooting expert. Provide clear, actionable insights based on the data provided."
                            },
                            {
                                "role": "user",
