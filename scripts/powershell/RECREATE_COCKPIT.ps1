# Script para recrear EconeuraCockpit.tsx limpio
# Uso: .\scripts\powershell\RECREATE_COCKPIT.ps1

$targetFile = "apps\web\src\EconeuraCockpit.tsx"

Write-Host "🔧 Recreando EconeuraCockpit.tsx limpio..." -ForegroundColor Cyan

# Eliminar archivo corrupto y backups
Remove-Item -Force "$targetFile" -ErrorAction SilentlyContinue
Remove-Item -Force "$targetFile.backup*" -ErrorAction SilentlyContinue

# Código fuente limpio (865 líneas)
$sourceCode = @'
...
# Escribir archivo
$sourceCode | Out-File -FilePath $targetFile -Encoding UTF8

Write-Host "✅ Archivo creado: $targetFile" -ForegroundColor Green
Write-Host "📊 Tamaño: $((Get-Item $targetFile).Length) bytes" -ForegroundColor Cyan

# Validar
Write-Host "`n🔍 Validando..." -ForegroundColor Cyan
pnpm -w lint 2>&1 | Select-String "EconeuraCockpit" | Select-Object -First 5
